...
class EzfaqController < ApplicationController
  ...
  def show_cat
    @category = FaqCategory.find_by_id(params[:cat_id])
    @faqs_in_category = Faq.find(:all, :conditions => "project_id = #{@project.id} and category_id = #{params[:cat_id]}")
    respond_to do |format|
      format.html { render :template => 'ezfaq/faqs_in_category.html.erb', :faqs_in_category => @faqs_in_category }
    end
    
  end
  
  def show_not_cat
     @faqs_in_category =Faq.find(:all, :conditions => "project_id = #{@project.id} and category_id is null and is_valid = true", :order => "question")
    respond_to do |format|
      format.html { render :template => 'ezfaq/faqs_in_category.html.erb',:faqs_in_category => @faqs_in_category }
    end
  end
